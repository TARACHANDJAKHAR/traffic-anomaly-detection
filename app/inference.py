...
def score_to_color(score, thresh):
    if score < thresh * 0.75:  return (0, 200, 0)    # green
    elif score < thresh:       return (0, 140, 255)  # orange
    else:                      return (0, 0, 220)    # red


def draw_box(frame, box, label, score, thresh, is_anomaly):
    x1, y1, x2, y2 = [int(v) for v in box]
    color = score_to_color(score, thresh)
...
def run_video_inference(video_path: str, output_path: str, bundle: dict,
                         target_fps: float = 5.0, progress_callback=None) -> list:
    ...
    t_scores = {}
    t_smoothed = {}
    t_anomaly_count = defaultdict(int)
    frame_log = []
    fidx = n_anom = 0
        ...
        if len(dets) > 0:
            ...
            if len(tracks) > 0 and tracks.tracker_id is not None:
                ...
                for i, tid in enumerate(tracks.tracker_id):
                    ...
                    # OUT-OF-DISTRIBUTION FIX: Scale to 640x480 domain for feature extraction
                    s_box = box * np.array([scale_x, scale_y, scale_x, scale_y])
                    s_others = others * np.array([scale_x, scale_y, scale_x, scale_y]) if len(others) > 0 else others

                    t_hists[tid].append(
                        update_track(t_states[tid], s_box, s_others, target_fps))
                    
                    score = t_scores.get(tid, 0.0)
                    smoothed = t_smoothed.get(tid, 0.0)
                    ...
                    if len(hist) >= MIN_FRAMES_FOR_LSTM:
                        ...
                        if seq_arr is not None:
                            ...
                            smoothed = (0.7 * smoothed + 0.3 * score) if smoothed > 0 else score
                            t_smoothed[tid] = smoothed
                            
                            # LATCHING: Require 3 consecutive frames of anomaly
                            if smoothed >= thresh:
                                t_anomaly_count[tid] += 1
                            else:
                                t_anomaly_count[tid] = max(0, t_anomaly_count[tid] - 1)
                                
                            if t_anomaly_count[tid] >= 3:
                                is_an = True
                                n_anom += 1
                                frame_anomaly = True
                                reason = "Trajectory anomaly"
                                
                    draw_box(frame, box, f"ID:{tid}", smoothed, thresh, is_an)
...
def run_image_inference(image_path: str, output_path: str, bundle: dict) -> dict:
    """
    Object detection only for a single image / frame.
    Temporal anomaly detection requires video.
    """
    from ultralytics import YOLO
    import supervision as sv
    ...
    if len(dets) == 0:
        ...
        for i in range(len(boxes)):
            ...
            # Draw neutral bounding box for normal detection
            color = (255, 140, 0) # Neutral blue/orange depending on BGR/RGB, let's use BGR: (235, 206, 135) or just standard (210, 150, 50)
            color = (210, 150, 50) 
            cv2.rectangle(ann, (x1, y1), (x2, y2), color, 2)
            
            lbl = f"Vehicle {conf:.2f}"
        ...
        # Legend
        yy = ann.shape[0] - 30
        cv2.rectangle(ann, (10, yy), (36, yy + 22), (210, 150, 50), -1)
        cv2.putText(ann, "Detected Vehicle", (44, yy + 16),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)

    cv2.imwrite(output_path, ann)
    return {"detections": detections_out, "n_anomalies": 0}
